...
export function PortfolioDashboard() {
  ...
  const fetchPortfolio = async () => {
      ...
      try {
        const res = await fetch(`http://127.0.0.1:8000/api/portfolio/${userId}`, {
  ...
  useEffect(() => {
    ...
    const socketUrl = `ws://127.0.0.1:8000/api/portfolio/ws/market`
        ...
        <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-1 flex items-center">
              <span className="relative flex h-2 w-2 inline-flex mr-2 items-center">
